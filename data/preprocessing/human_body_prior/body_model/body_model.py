...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        if '.npz' in bm_fname:
            ...
            with open(bm_fname, 'rb') as file:
                    smpl_dict = pickle.load(file, encoding='latin1')
        else:
        ...
        self.comp_register('shapedirs', torch.tensor(to_np(shapedirs), dtype=dtype), persistent=persistant_buffer)
        ...
        self.comp_register('J_regressor', torch.tensor(to_np(smpl_dict['J_regressor']), dtype=dtype), persistent=persistant_buffer)

        # Pose blend shape basis: 6890 x 3 x 207, reshaped to 6890*30 x 207
        if use_posedirs:
            posedirs = smpl_dict['posedirs']
            posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
            self.comp_register('posedirs', torch.tensor(to_np(posedirs), dtype=dtype), persistent=persistant_buffer)
        else:
            self.posedirs = None
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, pose_jaw=None, pose_eye=None, betas=None,
        ...
        if self.use_dmpl:
            ...
            shape_components = betas.squeeze(1)
